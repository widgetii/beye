     ...
     Это  руководство  документирует  внутренюю  архитектуру  BIEW   и
...
проект, равно как и удален из проекта. Как средство  взаимодействия  с
...
текстов проекта. Коротко о взаимодействии:
 ...
 -  При  старте  программы  управление  попадает   в   функцию   main,
  определенную  в  biew.c.  Здесь   программа   инициализирует   себя,
  анализирует командную строку, читает файл .ini, создает общие  окна,
  инициализирует плагины  и  адоны   и   передает  управление  в  файл
  mainloop.c.
 - В модуле mainloop.c определен  основной  цикл  обработки  сообщений
...
1.2. Основы плагинов и аддонов
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   Сегодня в проете существуют три класса плагинов:
- авто уровня
- I уровня
- II уровня
...
программой и и  плагинами  остальных  уровней  и  содержать  некоторые
...
нее.  Они  всегда  выбираются  через  меню  по  желанию  пользователя.
...
как средство борьбы с ошибками стандартных библиотек и  несоответствия
свойств их функций документации. Некоторые части biewlib были  рождены
как средство реабилитации мертвых и заброшенных  библиотек. Для  этого
...
- pmalloc -  реализует некоторые полезные механизмы работы  с  памятью
             ...
             нашел никаких средств  в  стандартном  наборе  библиотек,
...
- twin     - оконная  библиотека  совместимая  по  логике   работы   с
             ...
             библиотеку окон - я  ее  значительно  расширил  и  сделал
             переносимой).
sysdep/    - все  что  находится  в   этом   подкаталоге   нужно   для
             реализации  низкоуровневого  интерфейса  к   операционной
             системе. Надеюсь - там все и  так  понятно  что  к  чему.
             ...
             write,   close.   Следовательно   fileio   (как   и   все
...
правильнее взять ее реализацию из гарантированно  работающего  проекта
...
=======================================
    ...
    В основном проект расширяется по трем направлениям:
...
--------------------------------
    ...
    Для создания нового плагина или аддона необходимо создать  обьект,
...
расширения возможностей таких обьектов, но  пока  выбрана  именно  эта
схема взаимодействия частей программы, к  тому  же  она  не  исключает
возможность  реализации  обьектов,  как   внешних   модулей   в   виде
...
функциональности допустимый при создании обьектов такого класса.
  В файле reg_form.h содержится обьявление интерфейсов,  и  их  полное
...
-------------------------
   ...
   Задача портирования проекта одна из наиболее легких.  Вся  системно
...
операционных систем. POSIX не может полностью реализовать все  сам  по
...
любой ОС и ЦПУ специфичной информации. Все что Вы  должны  сделать  во
...
которых можно серьезно ускорить проект:
- __MsGetPos
...
изменении состояния  наблюдаемого  (этой  функцией)  обьекта,  которые
изменяют свое состояние асинхронно.
...
там и существуют свои  особенности,  хотя  я  убежден  что  и  на  тех
...
это еще не означает, что  программа  не  будет  запускаться  на  более
ранних моделях процессоров.  Во-вторых  уменьшение  размера  кода  (за
счет  подобной  оптимизации) всегда   приводит   к   более   выгодному
использованию  процессорных  кешей  и  в  третьих  еще  не  факт,  что
конвейеры и  предсказания  сводят  на  ноль  эффект  оптимизации,  они
минимизируют влияние cdecl но не так, что разницы не остается совсем.
   Как бы там  не  было,  в  коде  определен  макрос  __FASTCALL__,  с
...
2.      Все символы - символы языка  C  (extern  "C"  символы  в  C++)
        ...
        (каждый размер округлен  в  большую  сторону  до  кратности  4
...
4.      Когда вовращается структура,  вызывающая  процедура  размещает
        пространство  в   стеке.   Адрес   размещенного   пространства
        передается  через  стек  непосредственно   перед   инструкцией
        вызова функции. При  воврате  из  вызова,  регистр  EAX  будет
        содержать адрес  размещенного  пространства  для  вовращаемого
...
1.      Символы имен не модифицируются, так  что,  они  не  украшаются
        начальными или конечными подчеркиваниями.
2.      Аргументы передаются  через  стек  справа  на  лево.  Так  что
        последний  аргумент  заносится  в  стек   первым.   Вызывающая
        процедура должна очистить стек от аргументов.
...
2.      Аргументы передаются через стек в обратном порядке.  Так  что,
        первый   аргумент   передается   первым,   второй   передается
        следующим, и т.д. Вызываемая процедура  должна  очистить  стек
        от аргументов.
3.      Значения  с  плавающей  точкой  возвращаются   также   как   и
        структуры. Когда вовращается структура,  вызывающая  процедура
        размещает   пространство   в    стеке.   Адрес    размещенного
        пространства  передается  через  стек  непосредственно   перед
        инструкцией вызова функции. При воврате из вызова, регистр  AX
        будет   содержать   адрес   размещенного   пространства    для
...
3.      В  некоторых  реализациях  небольшие  структуры   возвращаются
        ...
        содержать адрес размещенного  пространства  для  возвращаемого
        значения.
...
- Минимизируйте использование          Избегайте частых разыменований указателей
  ...
  логикой программы.                   ющих связых между указателями, такое ра-
...
- Используйте модификатор "const".     Используйте классификатор типа "const"
                                       ...
                                       пространство под обьект, который обьявлен
...
--------------[ N/A ]----------------  Обьявляйте локальные функции как "static"
                                       Функции, которые не используются за
                                       пределами файла, в котором они определены
                                       всегда должны обьявляться статическими,
...
- Для лучшей производительности,      Обсуждение струтурных компонент языка "C"
  ...
  массивов, которые больше 32 байт,      базовым типам, обьявляя члены с боль-
...
- ВЫРАВНИВАНИЕ ДАННЫХ В СТЕКЕ          Сортируйте локальные переменные в соот-
  ...
  доступ к 64-х битовым переменным,    обьявлены в исходном кодеe, может быть
  которые не выравнены на границу 8    полезно обьявить локальные переменные в
  байт будет стоить 3 цикла процессора.таком стиле, чтобы переменные с большим
  На процессорах семейства P6, такой   размером базового типа обьявлялись впере-
...
- Разворачивайте все все короткие      Полное разворачивание циклов снижает наг-
...
- Воспользуйтесь преимуществом SIMD    Используйте 3DNow! инструкции:
  ...
  ее расширениями.                     в противном случае, выполняйте вычисления
...
- Избегайте частичного проставания     Избегайте частичного чтения и записи:
  ...
  отработки (8- или 16-ти разрядных)   записи частичного регистра обьединяет
  регистров, содержащихся в больших    измененная часть с текущим состоянием
  ...
  Следующая имеет доступ с последу-    Inst 2 обьединяет новый AH с текущим
...
- Динамический захват памяти посредс-  Соглашения динамического захвата памяти:
  ...
  равнял duobleword или quadword зна-  затель выравненный для наибольшего базо-
...
- Используйте EMMS инструкцию в конце  FEMMS инструкция дожна быть использована
  ...
  произойдет переход на вычисления с   оптимально работать на AMD-K6 процессоре.
...
- Убедитесь где это возможно что вы-    --------------[ N/A ]----------------
  ...
  водят к серьезным потерям.

- Спланируйте код на ассемблере ис-    Используйте FXCH инструкцию чаще чем пару
  пользуя FXCH инструкции. Когда воз-  FST/FLD:
  ...
  ризирйте код.                        FST/FLD парами, даже если FST/FLD пара
                                       ...
                                       бавляеь два такта ожидания состоит из
                                       двух операций.
...
результате появилась приведенная ниже сравнительная таблица.

Итак ТЕСТ:
   Все   замеры   проводились   на   одном   и   том   же   компьютере
(AMDK6-200/128Mb). Меня интересовали не  абсолютные,  а  относительные
цифры. (Еще лучше было-бы проводить  подобные  тесты  на  i386-40,  но
...
втором запуске запись идет в файл уже находящийся в кеши.  Эти  8  sec
...
серьезный продукт), пример с этой  системой  говорит  о  том,  что  не
...
оптимизации проекта. При  чем  cdecl  оптимизированный  проект  выдает
